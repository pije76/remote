...
#    re_path(r"^(?P<slug>[-\w]+)/$", CategoryView.as_view(), name="project_list"),
    re_path(r'^(?P<category>[-\w]+)/(?P<slug>[-\w]+)-(?P<pk>\d+).(?P<format>html)$', views.project_detail, name='project_detail'),
#    path('<category:category>/<slug:slug>/<int:pk>', views.project_detail, name='project_detail'),
#	re_path(r'^category/(?P<path>.*)', mptt_urls.view(model='project.models.Project', view='project.views.project_detail', slug_field='slug'), name='project_detail'),
#    re_path(r'^(?P<category>[-\w]+)/(?P<path>[-\w]+)/$', mptt_urls.view(model='project.models.Project', view='project.views.project_detail', slug_field='slug', trailing_slash=True), {'extra': 'You may also pass extra options as usual!'}, name='project_detail'),
